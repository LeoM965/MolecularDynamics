...
int main() {
    try {
        ...
        md.export_molecule_topology("output/topology.csv");

        md.add_thermal_velocities(300.0);

        md.export_xyz("output/initial_structure.xyz", 0);

        md.print_status(0);
        cout << "\nInitial temperature: " << md.calculate_temperature() << " K\n";

        md.export_energy_data("output/energy.csv", 0);
        md.export_bonds_angles("output/bonds_angles.csv", 0);
        md.export_csv_trajectory("output/trajectory.csv", 0);
        md.export_lj_forces("output/lj_forces.csv", 0);
        ...
        for (int step = 1; step <= total_steps; ++step) {
            ...
            if (step % export_interval == 0) {
                md.export_energy_data("output/energy.csv", step);
                md.export_bonds_angles("output/bonds_angles.csv", step);
                md.export_csv_trajectory("output/trajectory.csv", step);
                md.export_lj_forces("output/lj_forces.csv", step);

                if (step % xyz_export_interval == 0) {
                    const string xyz_filename = "output/structure_step_" + to_string(step) + ".xyz";
        ...
        md.export_xyz("output/final_structure.xyz", total_steps);
